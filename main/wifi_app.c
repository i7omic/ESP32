    ...
    if(event_base == WIFI_EVENT){
        switch (event_id){
            ...
            case WIFI_EVENT_AP_STACONNECTED:
                ESP_LOGI(TAG, "WIFI_EVENT_AP_STAYCONNECTED");
                break;

            case WIFI_EVENT_AP_STADISCONNECTED:
                ESP_LOGI(TAG, "WIFI_EVENT_AP_STAYDISCONNECTED");
                break;
...
/**
 * Initializes the TCP stach and default WiFi configuration
*/
static void wifi_app_default_wifi_init(void) {
    // Initialize the TCP stack
    ESP_ERROR_CHECK(esp_netif_init());

    // Default WiFi config - opperations must be in this order!
...
static void wifi_app_soft_ap_config(void) {
    ...
    ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap));                   // Start the AP DHCP server (for connecting e.g. your mobile device)
    ...
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(WIFI_IF_AP, WIFI_AP_BANDWIDTH)); // Our default bandwidth to 20 MHz
...
void wifi_app_start(void){
    ...
    xTaskCreatePinnedToCore(&wifi_app_task, "wifi_app_task", WIFI_APP_TASK_STACK_SIZE, NULL, WIFI_APP_TASK_PRIORITY, NULL, WIFI_TASK_CORE_ID);
}