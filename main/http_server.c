...
*/
const esp_timer_create_args_t fw_upadte_reset_args = {
    .callback           = &http_server_fw_update_reset_callback,
    .arg                = NULL,
    .dispatch_method    = ESP_TIMER_TASK,
    .name               = "fw_update_reset"
};
esp_timer_handle_t fw_upadte_reset;
...
static void http_server_fw_update_reset_timer(void)
{
    if(g_fw_update_status == OTA_UPDATE_SUCCESSFUL)
    {
        ...
        // Give the web page a chacne to receive an acknowledge back and initialize the timer 
        ESP_ERROR_CHECK(esp_timer_create(&fw_upadte_reset_args, &fw_upadte_reset));
        ESP_ERROR_CHECK(esp_timer_start_once(fw_upadte_reset, 8000000));
    }
    else
    {
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        // If so, it will have the information in the handler that we need
        if (!is_req_body_started)
        {
            ...
            printf("http_server_OTA_upadte_handler: OTA file size: %d\r\n", content_length);
            ...
            {
                printf("http_server_OTA_update_handler: Writing to partition %d at offset 0x%lx\r\n", update_partition->subtype, update_partition->address);
            }
    ...
    // We won't update the global variables throughout the files, so send the message about the status
    if(flash_successful)
    {
 ...
 * and responds with the compiler time/date when the page is first requested
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
*/
...
/**
 * wifiConnectStatus handler updates the connection status fro the web page
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
*/
...
static httpd_handle_t http_server_configuration(void)
{
    ...
    if (httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        // register query (jquery.js) handler
